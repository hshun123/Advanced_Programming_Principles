...
(* I changed my transform3 and transform4 functions. I used filter and map function to improve the readability and reduced the length of the code. 
And, for transform4 I used <> operator int filter to remove the empties *)
...
(* Use <> operator and List.filter to improve the code. Code from YongMin Jung *)

let transform3 (st3: string list) : string list =List.filter (fun a -> a <> "") st3

(* transform4 function *)

(* Use List.map to reduce the code length, improve readability. Code from YongMin Jung *)

let transform4 (st4:string list) : (string * int) list = List.map (fun a -> (a,1))st4 
